...
const offers = [
  {
    ...
    description:
      "Publish health and wellness AI artifacts in a structured environment designed for visibility, review, and early relevance testing.",
    extraTopClass: "",
  },
  {
    number: "02",
    ...
    description:
      "Explore how a prototype behaves in realistic communication and decision environments before anyone assumes it is deployment-ready.",
    extraTopClass: "lg:mt-24 mt-0",
  },
  {
    number: "03",
    title: "Structured Validation",
    description:
      "Evaluate usefulness, context fit, and practical viability alongside technical performance so stronger ideas move forward.",
    extraTopClass: "",
  },
  {
    number: "04",
    ...
    description:
      "Selected prototypes may extend into research demonstrations or educational content through Top Health AI Studio.",
    extraTopClass: "lg:mt-24 mt-0",
  },
];
  ...
  return (
    ...
    >
      ...
      <div className="2xl:w-full max-w-screen-2xl w-[95%] grid sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-4 lg:gap-3 items-start justify-center mt-4">
        {offers.map((offer) => (
          <div
            ...
            className={`w-56 bg-slate-200 text-white text-center mx-auto ${offer.extraTopClass}`}
          >
            ...
            <div className="w-0 h-52 z-10 mx-auto relative border-l-[15.6rem] border-l-lightBrown border-solid border-t-[0.6rem] border-t-transparent border-b-[0.6rem] border-b-transparent">
              <div className="w-[13.2rem] h-full pt-12 translate-x-[-14.3rem] absolute overflow-hidden bg-lightBrown text-brown px-2">
                <h1 className="font-bold text-lg">{offer.title}</h1>
                <p className="pt-1 text-sm">{offer.description}</p>
              </div>
            </div>
      ...
      <div className="flex flex-col items-center justify-center mt-16">
        ...
        >
          Exclusive Onboarding <br /> Schedule Now
        </Link>
      </div>
    </div>
  );
